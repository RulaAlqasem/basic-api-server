...
describe('Test delete /', () => {
  ...
  test('delete /example succeeds (204 OK) ', async () => {
    ...
    return supertest(server).delete(`/clothes/9-9-9`).send(serialise(toSend)).expect(204);
  });
});
...
describe('Test ubdate /', () => {
  ...
  test('put /example succeeds (200 OK) ', async () => {
    ...
    return supertest(server).put(`/clothes/9-9-9`).send(serialise(toSend)).expect(200);
  });
});